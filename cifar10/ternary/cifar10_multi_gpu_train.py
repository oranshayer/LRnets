...
tf.app.flags.DEFINE_string('train_dir', '/home/oran/logdir/cifar10_ternary',"Directory where to write event logs and checkpoint.")
tf.app.flags.DEFINE_integer('epochs', 290, "Number of epochs to run.")
...
"""
Ternary_large: wd: 0.000000000. wd_weights=0.0001. Dropout=0.5 . LR decay: 100, Precision: 91.9%
Ternary_large_2: wd: 0.00000000001. wd_weights=0.00001. dropout=1. LR decay: 170, Average: 91.4, best: 92.1 %
Ternary_large_3: wd: 0.000000000. wd_weights=0.00001. dropout=1. batch: 128
"""
...
def tower_loss(scope, images, labels):
  ...
  logits = cifar10.inference(images, None, None, None, None, None, None, None, train=True)
...
#        a_, b_, image_batch_ = sess.run([a, b, image_batch])
#        # LAYER 1
#        x = image_batch_[0,2:5,2:5,:]
#        activations = []
#        for i in range(3000):
#            W = cifar10.draw_ternary_weight(a_[:,:,:,10:12], b_[:,:,:,10:12])
#            W = W[:,:,:,0]
...
#        mu_ = a_[:,:,:,10] - b_[:,:,:,10]
#        mu_bar = np.sum(x*mu_)
#        sigma = a_[:,:,:,10] + b_[:,:,:,10] - mu_*mu_ + 0.001
#        sigma_bar = np.sum(x*x*sigma)
#        samples = np.random.normal(mu_bar, np.sqrt(sigma_bar), 3000)
#
...
##        jsd1+=[js]
##        steps1+=[step]
#        plt.title('Samples from a gaussian approximation')
#        plt.show()
