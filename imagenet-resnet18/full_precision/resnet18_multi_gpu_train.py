...
tf.app.flags.DEFINE_string('train_dir', '/home/oran/logdir/resnet18_3',
                           """Directory where to write event logs """
                           """and checkpoint.""")
tf.app.flags.DEFINE_integer('max_steps', 93000,
                            """Number of batches to run.""")
tf.app.flags.DEFINE_integer('num_gpus', 2,
                            """How many GPUs to use.""")
tf.app.flags.DEFINE_boolean('log_device_placement', False,
                            """Whether to log device placement.""")
tf.app.flags.DEFINE_boolean('new_run', False,
...
def train():
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    num_batches_per_epoch = (resnet18.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN /
                             (FLAGS.batch_size * FLAGS.num_gpus))
    decay_steps = int(num_batches_per_epoch * resnet18.NUM_EPOCHS_PER_DECAY)

    # Decay the learning rate exponentially based on the number of steps.
    lr = tf.train.exponential_decay(resnet18.INITIAL_LEARNING_RATE,
                                    global_step,
                                    decay_steps,
