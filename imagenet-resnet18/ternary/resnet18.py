...
tf.app.flags.DEFINE_float('wd', 1e-12,"""Number of images to process in a batch.""")
...
tf.app.flags.DEFINE_string('hot_start_dir', '/home/oran/logdir/resnet18_3/weights',"")
...
def initializer(scope, shape, prob):
    if FLAGS.hot_start:
        c_init, a_nz_init = init_values_hs_ver2(np.load(FLAGS.hot_start_dir+'/W_'+scope.name.replace('/','_')+'.npy'))
    else:
        c_init = init_probs(np.random.uniform(0.45, 0.55, shape))
        a_nz_init = init_probs(np.random.binomial(1,p=0.5*np.ones(shape)))
    if prob=='c':
        return c_init
    else:
        return a_nz_init
...
#        n = shape[0]
#      res = normal_dist.sample(tf.shape(mu_bar))*sigma_bar/np.sqrt(n) + mu_bar
...
def inference(images, W1, W2_1_b2a, W2_1_b2b, W2_2_b2a, W2_2_b2b,
  ...
  with tf.variable_scope('softmax_linear') as scope:
    ...
    weights = _variable_with_weight_decay('weights', [512, NUM_CLASSES], wd=FLAGS.wd_weights, layer_name=scope.name+'_w')
